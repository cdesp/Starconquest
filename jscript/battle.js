...
//  alert(prmxy.qx+':'+prmxy.qy);  
    ...
    if (shp != -1) {
        ...
        {
            alert('There is a fleet there choose another position');
            return 0;
        }
    }
...
function domousemove(mx, my) {
    ...
    grd = [0, 0];
    if (typeof pregrd == 'undefined')
        pregrd = [0, 0];
    if (!getgridcoords([mx, my], grd))
        return 0;
...
//add the div  
    ...
    if (divparent == null) {
        alert('div batleround not found');
        return false;
    }
    var temp = document.createElement('div');
    divship = document.getElementById('ships_' + fltid);
    if (divship == null)
        return 0;//ship destroyed???
    ...
    $('.ships').each(function (i, obj) {
        ...
        for (i = 0; i < shipsarr.length; i++)
            if (shipsarr[i].fltid == fltid) {
                ...
                if (stypearr[stid].maxsize == maxsize)
                    hullstid = stid;//image of the biggest
...
//  	alert('new battle '+batid);	
    ...
    if (myelm != null) {
        if (oldbgcolor == null)
            oldbgcolor = myelm.style.backgroundColor;
...
function getquadrfromxy(x, y, prmxy) {
    ...
    if ((prmxy.qx < 1) || (prmxy.qx > bgquadrh) || (prmxy.qy < 1) || (prmxy.qy > bgquadrv)) {
        prmxy.qx = 0;
        prmxy.qy = 0;
    }
