...
function mycostfunc(curnode, neighbornode) {
    ...
    x1 = Math.floor(x1 / solsyssize);
    y1 = Math.floor(y1 / solsyssize);
    x2 = Math.floor(x2 / solsyssize);
    y2 = Math.floor(y2 / solsyssize);

    soldist = (Math.abs(x1 - x2) + Math.abs(y1 - y2)) * 30;
    if (soldist > 0)
        soldist = Math.floor(Math.sqrt(soldist));
    return 1 + soldist * 30;
...
function showRoutes(fltid) {
    ...
    $.each(routearr, function (index, value) {
        if (typeof fltid != 'undefined' && fltid != value['fltid'])
            return true;
        rtid = value['rtid'];
        ...
        $.each(waypoints, function (index, value) {
            ...
            if (index == 0)
                objPre = {x: objReal.x, y: objReal.y};
            else {
...
function setEvents() {
    ...
    $('.fleets').on('click', function (e) {
        ...
        if (fltid == selfleet || f) {
            if (fcnt > 1 || f)
                showAllFleets(fleetarr[fltid].coordx, fleetarr[fltid].coordy);
        } else {
            selfleet = fltid;
            //set server session variable	
